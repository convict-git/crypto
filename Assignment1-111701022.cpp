...
/* Each SBox is 6 to 4, i.e. 6 bit input to 4 bit output */
const int IN_BITS = 6, OUT_BITS = 4;

/* Difference Distribution Table (DDT) : 8 x 2^6 x 2^4
  ...
  for (int sbox_idx = 0; sbox_idx < 8; ++sbox_idx) {                // for each Sbox out of 8
    for (int X = 0; X < (1 << IN_BITS); ++X) {                      // Fix some X from [0, 2^6 - 1]                                                 
      ...
      for (int Xdash = 0; Xdash < (1 << IN_BITS); ++Xdash) {        // Fix some X' from [0, 2^6 - 1]
        int Ydash = Sbox[sbox_idx][row_idx(Xdash)][col_idx(Xdash)]; // Get Y' corresponding to the above fixed X' and Sbox

        int deltaX = XOR(X, Xdash);                                 // Calculate deltaX = X xor X'
        int deltaY = XOR(Y, Ydash);                                 // Calculate deltaY = Y xor Y'
        DDT[sbox_idx][deltaX][deltaY] += 1;                         // Increase the frequency deltaY for the corresponding deltaX and Sbox
      }
    }
  }
}
